                 ...
                 fluidRow(
                   shinydashboard::box(
                     ...
                     fluidRow(
                       ...
                       column(6, img(src = "image4.jpg", height = "110%", width = "110%"))  # Replace with the actual image path
                     ),
                     br(),
                     br(),
                     br(),
                   ...
                   shinydashboard::box(
                     ...
                     fluidRow(
                       column(width = 6,
                              ...
                              shinyAce::aceEditor("code_editor0", 
                                                  mode = "python", 
...
column(width = 6,
       ...
       shinyAce::aceEditor("code_editor9", 
                           mode = "python", 
                           theme = "tomorrow_night_bright", 
                           height = "320px",
                           value = "
...
fluidRow(column(width = 6,
                DT::DTOutput("table_output0")),
         (column(width = 6,
                 DT::DTOutput("table_output9")))
),
...
fluidRow(
  column(width = 6,
         shinyAce::aceEditor("code_editor1", 
                             mode = "python", 
                             theme = "tomorrow_night_bright", 
                             height = "280px",
                             value = "
...
fluidRow(
  column(width = 6,
         shinyAce::aceEditor("code_editor2", 
                             mode = "python", 
                             theme = "tomorrow_night_bright", 
                             height = "270px",
                             value = "
...
fluidRow(
  column(width = 6,
         shinyAce::aceEditor("code_editor3", 
                             mode = "python", 
                             theme = "tomorrow_night_bright", 
                             height = "270px",
                             value = "
...
fluidRow(
  column(width = 6,
         shinyAce::aceEditor("code_editor4", 
                             mode = "python", 
                             theme = "tomorrow_night_bright", 
                             height = "290px",
                             value = "
...
# Example API call and data processing
url = 'https://jmmi-db-2024-108.azure-api.net/impact-jmmi-api-open/full_data'
params = {
    'country': 'CAR',
    'start_date': '2020-04-01',
...
# Assuming the JSON data has a 'data' key containing the records
df = pd.DataFrame(data['data'])

# Convert all columns to string to ensure compatibility when converting to R
df = df.astype(str)

# Print the first few rows for debugging
print(df.head())

# Save DataFrame to global environment for R to access
import builtins
builtins.df = df
...
server <- function(input, output, session) {
  ...
  process_request <- function(editor_input, spinner_id, output_id) {
    observeEvent(input[[paste0('send_request', gsub("code_editor", "", editor_input))]], {
      ...
      # Show spinner
      shinyjs::runjs(paste0("$('#", spinner_id, "').show();"))
      ...
      tryCatch({
        reticulate::py_run_file(temp_code_file)
        
        df <- reticulate::py$df
        
        df <- reticulate::py_to_r(reticulate::py$df)
        
        
        # Print the structure and first few rows of the dataframe for debugging
        print(str(df))
        print(head(df))
        ...
        if (nrow(df) == 0) {
          showNotification("No data returned. Try adjusting the query parameters.", type = "warning")
        }
        ...
        shinyjs::runjs(paste0("$('#", spinner_id, "').hide();"))
